...
printed (the attribution battery) are descriptive companions to an effect size.
...
WHAT THE ATTRIBUTION BATTERY FOUND — the acoustic channel does NOT measure
the athlete. Splitting nights by whether one was slept alone or with a
co-sleeper present (classification supplied by the athlete; see
COSLEEPER_CONFIG), P(snore>0) is 0.120 alone vs 0.674 accompanied: risk
difference +0.554, 95% bootstrap CI (+0.316, +0.752), clearing the
pre-registered +0.30. It holds in BOTH instrument regimes separately and
AMBIENT NOISE IS IDENTICAL between the groups within each regime — so the
away rooms were not merely quieter, and the microphone's environment held
constant while the bed's occupancy changed. Alcohol, which reliably
increases snoring in the DRINKER, produces no rise on the athlete's own
tagged nights. The channel is real and it is not the athlete's. Per S4 that
verdict covers `Breathing disruptions` too, since it is the same 180 nights.

The one the athlete expected to survive did not, but not for the expected
reason: `Movements per hour` is NOT co-sleeper-contaminated (alone vs
accompanied p=0.43) — it is simply uncorrelated with Oura restless periods
(rho=0.111, CI spanning zero) at ~3x the rate. Two counting rules on two
detectors.
...
THRESHOLDS = {
    ...
    # Roughly the gap between "mostly the co-sleeper" and "mostly shared".
    "cosleeper_risk_difference": 0.30,
...
# Whether a night was slept alone is NOT derivable from the export -- it is
# supplied by the athlete. It is read from a gitignored file beside the export
# rather than hard-coded, because a home location and a travel history are
# identifying details that do not belong in a source repository, and because
# this repo is public. Format, {"alone": [...], "accompanied": [...]}, values
# matched against the export's own `City` column:
#     Input_files/sleepcycle_cosleeper.json
# Absent -> section 3's attribution battery reports UNTESTABLE and says so,
# which is the correct reading: without it there is no contrast to draw.
COSLEEPER_CONFIG = ROOT / "Input_files" / "sleepcycle_cosleeper.json"


def _cosleeper_groups() -> tuple[set[str], set[str]]:
    if not COSLEEPER_CONFIG.exists():
        return set(), set()
    try:
        cfg = json.loads(COSLEEPER_CONFIG.read_text(encoding="utf-8"))
    except (ValueError, OSError) as exc:
        # Absent is expected and reads as UNTESTABLE. MALFORMED is a different
        # fact and must not be laundered into the same message -- a broad
        # `except Exception` here already hid a missing `import json` once.
        raise SystemExit(f"{COSLEEPER_CONFIG} is unreadable: {exc}")
    return set(cfg.get("alone") or []), set(cfg.get("accompanied") or [])
...
ALONE, ACCOMPANIED = _cosleeper_groups()
solo = [r for r in all_rows if (r.get("City") or "").strip() in ALONE]
partner = [r for r in all_rows if (r.get("City") or "").strip() in ACCOMPANIED]
if not solo or not partner:
    print(f"  ATTRIBUTION UNTESTABLE — no co-sleeper classification found at")
    print(f"  {COSLEEPER_CONFIG.relative_to(ROOT)}. Without it there is no contrast")
    print(f"  to draw, and the acoustic channel cannot be attributed to anyone.")
    solo = partner = []
else:
    print(f"  Nights slept ALONE: {len(solo)}      "
          f"nights with a CO-SLEEPER present: {len(partner)}")
    print()
    print(f"  {'channel':<24}{'alone median':>14}{'co-sleeper med':>16}{'alone %zero':>13}"
          f"{'co-slp %zero':>14}{'MWU p':>10}")
...
if solo and partner:
    ...
    print()
    print(f"  P(snore>0):  alone {st.mean(sa):.3f}   accompanied {st.mean(pa):.3f}")
    print(f"  RISK DIFFERENCE {rd:+.3f}  ~95% CI ({rd_lo:+.3f}, {rd_hi:+.3f})   "
          f"threshold >= {THRESHOLDS['cosleeper_risk_difference']:.2f} with CI excluding 0")
    attributed = rd >= THRESHOLDS["cosleeper_risk_difference"] and rd_lo > 0
    print(f"  -> NOT THE ATHLETE'S: {'CONFIRMED' if attributed else 'not confirmed'}")

    print()
    print("  Stratified by instrument regime (S6) — the confound that could "
          "have manufactured this:")
for reg in ("old", "new") if (solo and partner) else ():
...
if solo and partner:
    print("    Ambient noise equal within each regime => the away rooms were "
          "not merely quieter.")
...
print(f"  NOT THE ATHLETE'S       snore, breathing disruptions "
      f"(S4: one detector) — RD {rd:+.3f}" if rd is not None
      else "  ATTRIBUTION            UNTESTABLE (no co-sleeper classification)")
