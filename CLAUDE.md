...
17. **In-app navigation NEVER uses an HTML anchor — new screens open IN the same page, never as a new one.** This app is an SPA: `app.py` routes on `st.session_state["_nav_page"]`, and every navigation must set state and let Streamlit rerun over the **existing** WebSocket. A real `<a href="?...">` does the opposite — a full browser navigation that reloads the page, reconnects the WebSocket, wipes `session_state` and leaves every `@st.cache_data` cold. On a phone that is seconds, and it reads as *"it opened a new page"*. Use `st.button(..., on_click=...)`, styled to look like whatever the design needs: `nav.py` styles buttons into the fixed bottom bar, and `views/insights.py`'s BioAge cards are a worked example of a button styled as a 150px image card (`st.button(key="x")` emits a `.st-key-x` class, which is the documented styling hook). **Carrying state in the URL is still fine and often right** — assigning to `st.query_params` sends a `page_info_changed` message and the frontend rewrites the URL via the history API with **no reload** (`streamlit/runtime/state/query_params.py::_send_query_param_msg`), which is why the Home screen can keep `d`/`view`/`pt` in the URL for reconnect survival (`app.py`'s own note) while still navigating by button. The anchor is the problem, not the query string. `tests/test_spa_navigation.py` enforces this, and as of 2026-08-10 its `_KNOWN` allowlist is **EMPTY** — `app.py`, `styles.py` and `views/` contain no in-app anchor at all, so any anchor now fails the gate. **Where no widget can be placed, use the bridge, not a link.** The chart hit bands are the case that forced it: `chart_hits()` returns an HTML string composed into a chart's own markdown, up to `dashboard.hit_bands`' 48 bands laid over a generated SVG, so there is no point in the element tree for a button. They are now `data-nav` spans, and `styles._CHART_LINK_JS` turns a click into (1) a `history.replaceState` on the query string and (2) a click on a hidden trigger button — whose rerun request carries the browser's **live** `location.search` into `ClientState.query_string` (`app_session.py`), so Python reads the new `?pt=` on that rerun with no reload. That last fact is undocumented and lives in minified frontend code, so it is asserted against a real Chromium in `tests/test_chart_nav_bridge.py` rather than reasoned about; that test skips (never fails) without playwright. A `data-nav` span needs `role="button"` and `tabindex="0"` — focusability is the one thing the anchor gave for free.

11. **Before authoring any new training block, explicitly confirm each local clinical profile document has been read** — `patient_profile.py` plus every `Input_files/*.md` document present — and state how each one influenced the plan, per `docs/clinical_profile_weighting.md`. This is the checkable form of "understood and acknowledged," not a formality to skip. **From 2026-08-10 this gate also covers `docs/training/warmup_evidence_review_2026-08-10.md`** — same standard, same checkable form: state how it influenced the plan, or say plainly that it did not and why. **Gated on the EVENT, not a date** (`HRV_GARMIN_HOLD` idiom): the plan is NOT being rebuilt on 2026-08-16 (athlete, 2026-08-10), so it waits for the block build rather than expiring with the Day 28 reassessment. It is in the gate rather than in a docs list because the next block is the first to run near-maximal loads and **the system contains no warm-up at all**; a gym day goes from the last release hold straight into the first working set. It also carries a hard software prerequisite — see the Known Open Issues row.
