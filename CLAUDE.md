...
Expected: **2903/2903 passed** (or higher — this count grows as tests are added; treat it as a floor, not an exact match. Measure the number for a commit message against the committed tree only — a shared working tree can carry another session's uncommitted tests. Re-measured on 2026-08-14 after the Stage 2B build (+194: the block's own invariants, the per-set fields, the phase route, and the parametrised coverage tests that pick a new plan up automatically). **A CONCURRENT SESSION IS THE NORMAL CASE, not the exception** — a reading of 2686 taken minutes earlier was correct for its own commit and looked like flakiness until `git log` showed another session had landed `tests/test_no_replay_unsafe_cached_elements.py` (+5) in between. Before diagnosing a changed count as non-determinism, run `git log --oneline -4`.)
...
| **⚠ A phase's `date_overrides` can schedule days past its own last date, and they vanish** | Found live 2026-08-14, fixed for Stage 2A, NOT fixed as a class. Reschedules move day numbers later; nothing lengthens the phase to match. `plan.stranded_override_days(phase)` reports the overhang and `_render_no_active_phase` warns about it, so it is now VISIBLE — but nothing PREVENTS it. The scheduling code that creates the overhang (`missed_reschedules`, `swap_pairs_for_shift`) already caps moves at the phase end for the day it is moving, yet the cumulative renumbering still walks the tail off the end. A real fix is either to extend `length_days` when a shift would strand content, or to refuse the shift — both change scheduling behaviour and neither should be done casually. **Check `stranded_override_days` at every block boundary until then.** |
