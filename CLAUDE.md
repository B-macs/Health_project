...
| Sleep Cycle (iOS) export — measured, then declined | **MEASURED 2026-08-05**, 296 nights 2024-10-31→2026-07-23, 43 paired vs Oura / 11 vs Garmin. It holds the **widest calendar coverage in the project — 245 nights no device recorded, longest run 46** — and the only acoustic channel. **No channel passed.** Time asleep agrees at bias +0.47h with 95% LoA (−0.50, +1.44) — three times the pre-registered 45-min half-width; everything else fails on bias, LoA or information ratio; staging reads **UNTESTABLE not failed** (n=19). Four structural facts decided most of it before any statistic: `Time in bed` ≡ `End − Start` to 0.00s (a hand-opened recording window, not a measurement) and Awake is its arithmetic residual; `Snore>0` and `Breathing disruptions>0` are the **identical 180 nights, zero off-diagonal** (one detector, two readouts); ambient noise steps ~23.5→~19.5 dB across late 2025 (**two phones — do not transfer a coefficient across it**, same prohibition as 645/265); and all 29 columns are night-level scalars, so nothing can enter `sleep_movement.py`'s 30s grid at ANY agreement level. **The acoustic channel is the PARTNER's**: across 25 solo vs 141 partner-present nights P(snore>0) is 0.120 vs 0.674, RD **+0.554, 95% CI (+0.316, +0.752)**, holding in both instrument regimes with ambient noise identical within each — and the athlete's own alcohol tag produces no rise. Movement is *not* partner-contaminated (p=0.42), merely uncorrelated with Oura (ρ=+0.24, CI spans 0). **Nothing in `services/` or `views/` reads any of it**, and per rule 2b + `sleep_fusion.py`'s shadow report, importing 245 uncalibrated nights against 43 calibrated ones would raise the 56-night sleep baseline exactly where the ring was not worn. Verdict **KEEP RECORDING, DO NOT INGEST**. Re-run `scripts/compare_sleepcycle_to_devices.py` (docstring carries the full tables — the CSV is gitignored, so it is their only durable record). |
| ⚠ There is no fixed sleep-duration reference — Oura and Garmin disagree by an hour | Found 2026-08-05 while measuring Sleep Cycle, **applies to the whole project, not just to that file.** Over the 26 `fused` nights Garmin reads **+1.11h more sleep than Oura (sd 0.64)**, and their minute-by-minute stage agreement is **52.3% at Cohen's κ 0.178** — "slight". Consequence for any future third-source evaluation: a bias measured against Oura will not replicate against Garmin, and can flip sign (Sleep Cycle is +0.47h vs Oura, −1.07h vs Garmin). **Never describe a candidate source's offset as an over- or under-estimate without naming the comparator.** This is also why `sleep_fusion` takes stage from Oura and only permission-to-call-Awake from Garmin rather than averaging them, and it strengthens rule 2b: mixing instruments into one rolling window is noise when the instruments that define the window already disagree this much. |
| Garmin holds sleep DURATION history but not stages — a real backfill opportunity | Probed 2026-08-05 over 2024-10-31→2026-05-18 (`backfill_garmin_sleep_stages.py --probe`, 10 sampled dates): **`sleepLevels` on 0/10, `sleepTimeSeconds` on 6/10**, reaching back to 2024-10-31. The watch was worn across that whole span; the local record starts 2026-05-19 only because that is when stage capture began (`garmin_daily.sleep_hours` starts 2026-06-28, 27/34 rows populated). So **a stage backfill buys nothing and is permanently impossible** — Garmin keeps the daily total and discards the hypnogram — but **a daily-duration backfill is available** and would feed the readiness baselines the "Garmin backfill" row below is about, as well as taking any Sleep-Cycle duration comparison from n=11 to potentially 150+. Check `sleepWindowConfirmationType` before trusting a historical total: an unconfirmed window is Garmin guessing. Note the account is near the IP rate limit — two mobile login paths returned 429 during the probe. |
