...
Expected: **2885/2885 passed** (or higher — this count grows as tests are added; treat it as a floor, not an exact match. Measure the number for a commit message against the committed tree only — a shared working tree can carry another session's uncommitted tests. Re-measured on 2026-08-14 after the Stage 2B build (+194: the block's own invariants, the per-set fields, the phase route, and the parametrised coverage tests that pick a new plan up automatically). **A CONCURRENT SESSION IS THE NORMAL CASE, not the exception** — a reading of 2686 taken minutes earlier was correct for its own commit and looked like flakiness until `git log` showed another session had landed `tests/test_no_replay_unsafe_cached_elements.py` (+5) in between. Before diagnosing a changed count as non-determinism, run `git log --oneline -4`.)
