...
| 36 Metrics History rows predate the background-sync commits | Found 2026-08-03 while scoping the nap re-derive, and deliberately NOT fixed. Every stored row before 2026-07-17 was computed by code older than the sync rework (`551323c`..`2b092ad`), so re-deriving them applies those commits too — which **blanks readiness on 16 rows across 2025-09-27 → 2025-10-13** that currently hold values. Those values are flat-repeated (91.2 for 11 straight days), the signature of the stale carry-forward `adec27d` ("a new day is blank until it has actually been measured") set out to fix, so blanking them is probably correct — but it is data loss on rows naps never touched, so it wants its own decision. Nap support alone moves nothing before 2026-07-17 (verified: 0 of 36). Fix with `sync_metrics_history(days=320)` if wanted. |
