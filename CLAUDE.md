...
Expected: **2920/2920 passed** (or higher — this count grows as tests are added; treat it as a floor, not an exact match. Measure the number for a commit message against the committed tree only — a shared working tree can carry another session's uncommitted tests. Re-measured on 2026-08-14 after the Stage 2B build (+194: the block's own invariants, the per-set fields, the phase route, and the parametrised coverage tests that pick a new plan up automatically). **A CONCURRENT SESSION IS THE NORMAL CASE, not the exception** — a reading of 2686 taken minutes earlier was correct for its own commit and looked like flakiness until `git log` showed another session had landed `tests/test_no_replay_unsafe_cached_elements.py` (+5) in between. Before diagnosing a changed count as non-determinism, run `git log --oneline -4`.)
...
18b. **THE WEEK IS ITSELF A BLOCK — a rescheduled day moves WITHIN its week or not at all** (athlete, 2026-08-14). The seven days are a unit of training design, not a convenience: two gym days, two runs, the flexibility slot and the spacing rules between them. A day that slides into the following week does not land in a week with room for it — it lands in one already carrying its own full complement, and pushes that week's last day into the week after. Enforced by `plan.reject_violating_overrides`, which **refuses rather than clamps** (a day number nudged to fit is a different session from the one the athlete was offered, silently substituted) and which **all three `date_overrides` write sites in `views/training.py` pass through** — the manual swap, the missed-session carry and the readiness auto-shift; a source test pins that a fourth cannot appear without it. Two rules, one function: no override may put a plan day in a different week of the block than its date falls in, and none may point past the block's last date. A `0` (forced rest) schedules nothing and is always allowed. **This is the rule Stage 2A did not have**: a forced rest on 2026-08-09 renumbered the tail, week 3's days 20 and 21 ran in week 4's slots, and the far end walked off the block's own last date taking the day-28 reassessment with it — see the Known Open Issues row. `services/scheduling.py`'s `_week_sunday` already bounded the day being MOVED; what was unbounded was the cumulative renumbering left behind, which is why the guard sits at the WRITE rather than at the generator.
...
| **✅ Session HR runs, and per-exercise RPE now comes from it — 2026-08-14** | **This row said the sync had NEVER run. That was stale.** `sync_session_hr_for_date` was repaired at some point after the row was written — it reads `get_session_sets_by_exercise(d)` rather than the non-existent `ex.sets`, and it is wired into `run_home_syncs` via `sync_session_hr_recent_if_due(days=2)`. Verified by data, not by reading: the 2026-08-10 session carries a full Session HR row **and per-exercise HR-derived RPE** — Upper Glute release **1.4**, Piriformis PNF **1.8**, Dead Bug **3.4**, Pallof hold **5.2**. The index-space warning is stale too: BOTH callers key `per_exercise` by MOVEMENT NAME (`get_session_sets_by_exercise` and `views/training.py::_sets_by_movement`), which that method's own docstring explains was done precisely so heart rate could not be attributed to the wrong movement. **What WAS still broken is what the athlete spotted: every exercise carried the SESSION slider value.** `views/training.py` passed `rpe=session_rpe` to every `save_training_exercise` call, so a 90-second pressure release and a set of RDLs both recorded RPE 8. Not cosmetic — `services/strength.py` takes `exercise_rpe` FIRST and falls back to `session_rpe` only when it is None, so the fallback was never reached and every 1RM estimate was computed at the session figure. Now: the save writes **None**, and `Repository.reassign_exercise_rpe_from_hr` fills in each exercise's own %HRR-derived value on the next HR sync, matched **by movement name**. It runs after the fact by necessity — the Garmin activity is not on Garmin's servers when the last set is logged. An exercise whose block has no samples (`covered` False) **keeps its null**: not-measured is a fact, and overwriting it with a guess is the failure this change undoes. Mirrors with `supabase_store.PATCH`, never UPSERT — a partial upsert would insert an orphan `training_exercises` row holding an RPE and nothing else. **⚠ The session slider is UNTOUCHED**: it is the athlete's own answer, it feeds `session_au` → Strain/ACWR, and key rule 2b keeps anything HR-derived away from that. Also fixed on the way: `sync_session_hr_for_date` returned a `needs_choice` dict straight into `save_session_hr`, which would `KeyError` on the missing `date` and surface as a failed sync rather than as "no match, fall back to RPE". |
| **⚠ A phase's `date_overrides` can schedule days past its own last date, and they vanish** | Found live 2026-08-14, fixed for Stage 2A, NOT fixed as a class. Reschedules move day numbers later; nothing lengthens the phase to match. `plan.stranded_override_days(phase)` reports the overhang and `_render_no_active_phase` warns about it. **PREVENTED as of 2026-08-14 by key rule 18b** — all three write sites refuse an override that crosses a week boundary or points past the block end, which is the cumulative renumbering that caused this. The reporting stays because the rule guards NEW writes and cannot repair a phase that already drifted. The scheduling code that creates the overhang (`missed_reschedules`, `swap_pairs_for_shift`) already caps moves at the phase end for the day it is moving, yet the cumulative renumbering still walks the tail off the end. A real fix is either to extend `length_days` when a shift would strand content, or to refuse the shift — both change scheduling behaviour and neither should be done casually. **Check `stranded_override_days` at every block boundary until then.** |
