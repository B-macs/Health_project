...
| **⛔ No per-set warm-up flag — now a BLOCKING PREREQUISITE, not a nicety** | `services/tonnage.py` counts a set as eligible when it carries reps AND a real external load (`if reps and weight`, line 155). Warm-ups are NOT excluded, because the log has no way to mark one — so "working sets only" is an assumption the data cannot currently support. A boolean per set closes it. **Status changed 2026-08-10:** this was cosmetic only while no warm-up existed. The next block introduces ramp sets (see the warm-up row above), and a ramp set carries reps and real load — so on the day it ships it would silently inflate **weekly tonnage**, feed `EXERCISE_MOVEMENT_WEIGHT` into **Strain/AU**, and pollute `services/strength.estimated_1rm`, whose 17 estimates are already outside Epley's validated range. **The flag lands before the first ramp set is authored, not after.** |
