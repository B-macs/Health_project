...
*Last updated: 2026-08-06 — the FLEXIBILITY sector shipped, and its **first model was refuted by the athlete and deleted** on the way. **Read `flexibility_baselines.py`'s and `services/flexibility.py`'s module docstrings before touching any of it.** v1 scored eight body regions as `sqrt(RANGE x CONTROL)` and averaged them; the athlete killed it in one sentence — *"we know my hips are stuck in flexion with my back arched and this is a huge issue for me, and yet my flexibility score is nearly 80 for hips"* — because the `hip` average buried Deep Lunge, the ONE thing testing his single worst documented capacity, under four healthy contributions. **v2 scores SKILLS, not regions.** A skill is a position you can either achieve or not; under each is a LADDER of candidate limiters, and the skill's score is **`min(rungs)`** with the name of that rung published beside it — fix it, re-test, watch the limiter move to the next rung, and that re-pointing IS the training programme. Regions are demoted to diagnostics and **nothing is ever averaged**. **EIGHT skills, and ONE is trained at a time** (athlete, 2026-08-06): pancake, pike & head-to-toe, front split, side split, squat, shoulder flexion, shoulder extension, bridge. **The target is chosen BEFORE the tests are taken**, and that ordering is the whole point — a limiting rung only means something against a goal. The athlete's objection killed the previous four: *"when you say Chest / pecs is the limiting factor — what skill am I working towards? Chest and pecs are only the limiting factor if I want to do a handstand or a bridge, but if my first goal is a pancake then chest and pecs wouldn't be that important compared to hamstrings."* The old four (deep squat, hip extension, shoulder flexion, active pike) were correct as LADDERS and wrong as GOALS — he can already hold a deep squat and nobody aims at "hip extension". **NO NEW RUNGS WERE NEEDED to fix it**, which is the evidence the failure was in the naming layer alone: hip extension became the back leg of a front split, shoulder flexion became elbows-to-the-floor. `Assessment.target_skill` records what he was aiming at on the day, so a later switch of target is legible instead of looking like the numbers moved for no reason; `flexibility.prescribe()` returns that skill's limiting rung plus ONLY the stack steps that move it, and `flexibility.compare()` feeds the re-test decision (stay on this skill and take the next rung, or switch and get a different ladder). **Pancake is the first target and the only skill with a stack built.** It is defined as the FLAT-BACK straddle — the conventional pancake finishes as a seated forward fold, which `rules.py` contraindicates outright; what makes the goal valuable here (opening from the hip) and what makes it dangerous (rounding the lumbar spine) are separable, and a test asserts no stack step anywhere contains a contraindicated movement name. Stacks are ORDERED AND CUMULATIVE and are built on the **assisted → resisted spectrum** — and for this athlete **no step may be assisted**: at Beighton 6/9 the assisted half of the source method solves a problem he does not have, so the work lives at the resisted end and a test pins it. **Bridge and shoulder extension are in the catalogue and NOT deleted** — they still score and still show regression — but they are `needs_signoff` and cannot be selected: the route to each runs through end-range lumbar extension and the post-Latarjet apprehension direction respectively. Both name 2026-08-16 as what unblocks them. **Three measures per rung** — PASSIVE (the ceiling), ISOMETRIC (is it defended), ACTIVE (usable range) — and `gap = passive − active` is the number that matters at Beighton 6/9: wide gap means the range exists and cannot be held, so stretching is the WRONG lever and resisted work the right one. That replaced v1's self-rated CONTROL axis, and with it the **two-sided `CONTROL_BAND` where a depth rating of 88 scored 46**. Achievement is now **monotonic — more is always better** — and the hypermobility concern lives in the gap where it is measured instead of assumed; `band_score`, `control_score`, `CONTROL_BAND`, `OVERSHOOT_SLOPE` and `UNDERSHOOT_EXPONENT` are gone and `test_the_refuted_two_sided_band_is_gone_and_stays_gone` fails loudly if any returns. Every test names a **LOCK** (the thing that must not move — an unlocked joint lets a neighbour substitute and the test measures nothing, which is what broke the model twice); six tests `replaces` a standard test contraindicated for this athlete; distances are taped, not angles eyeballed, because a distance is what one person alone reproduces in three months; and everything is **measured COLD**, never warmed up, or the reading tracks whether he stretched that morning rather than durable change. The 14th rung, `lats`, closed a real hole: three tissues stop an overhead reach and only the lat crosses the lumbar spine, so changing pelvic tilt changes lat tension and nothing else — without that isolation an overhead test says "the arm does not go up" but not which tissue stopped it, and post-Latarjet the capsular answer makes aggressive stretching wrong rather than merely useless. **`ASSESSMENTS` is empty and that is the honest state**: zero of the 22 legacy pose ratings survive into v2 (they answer neither question), and `LEGACY_GYM_READINGS` / `LEGACY_POSE_DEPTH_RATINGS_2026_08_05` are kept as provenance that nothing computes from. Capture is a resumable **14-step flow** with durable drafts in `local_cache` — one test per step, wrapped in `st.form` because `st.number_input` does not commit until blur and a full run silently lost the `neck` rung before that. Count DISTINCT rungs, not readings: a bilateral test writes two readings for one rung and displayed "19 of 14". Staleness decays WEIGHT, never VALUE. Display-only — **nothing here reaches the engine**, same standing rule as Body Composition. Also shipped: a second yoga (`shoulder_scapula_neck_flow_16min`, 16 poses, scapular holds deliberately under 30s so it cannot pre-empt the physio's 2026-08-16 decision on endurance-biased scapular work), `suggest_for_day` rebuilt from a first-match filter into a real ranking, and a `YogaPose.retest` field carrying three open questions back to the poses that can answer them. Gate 1515 -> 1584. **Two things decided but NOT built**: the rest-day conflict (the app offers yoga on rest days, the window this model calls worst for adaptation) resolves as **option 2, an `intent` field — `restorative` | `training` — on `YogaSession`**, deferred until the training-schedule overhaul; and three frozen constants (acromion-to-styloid length, shin length, a traced foot outline) are not yet captured, which three tests need. Before that, the first FLEXIBILITY baseline landed and corrected two ways this repo had been misreading its own clinical data. The athlete self-rated all 22 poses of `services/yoga.py`'s hip/spine flow (1-100, 1 = can barely enter it, 100 = at the physical limit); the measured table lives in `docs/training/Yoga_Library.md`, the synthesis in `patient_profile.py` `symptom_log` 2026-08-05, and the dosing consequence in `docs/training/physio_brief_2026-08-16.md` §11. **A Beighton score is a laxity screen, not a ROM measurement** — reading palms-flat-to-floor as hamstring length over-predicted the straddle fold by 55 points, when the real (and dominant) restriction is an inability to reach anterior pelvic tilt in sitting. **`imbalances.overactive_tight` is resting TONE, not shortness** — reading it as restricted range under-predicted 90/90 hip rotation by 52 points. Also: finding #4's Coxa Saltans trigger is CONTRACTILE not positional (no snap in any passive flexion+ER position), the documented right>left asymmetry does not appear in passive stretch at all, and interscapular fatigue onset under bodyweight is measured at 50-60s — the first quantified number for the endurance gap going to physio on 2026-08-16. One pose was misidentified: "Spine Mobilisation" is a seated cross-legged side bend, was tagged `cleared`, is now `caution` and caught by a new `side bend` keyword in `services/rules.py` with its own test in `tests/test_rules.py`. Before that: the Metabolism BioAge screen shipped: `services/body_composition.py`, `body_composition_baselines.py` and `views/insights.py::_render_metabolism_detail`, with `docs/resume.md` gaining a BODY COMPOSITION section for its locked decisions. Two devices are kept in permanently separate lanes — a Foryond scale whose fourteen columns are one measurement (its body fat percent is fitted from weight and age at R^2 0.9966) and an InBody 770 whose five scans were run against four different typed heights, corrected by `InBodyScan.at_height`. Gate 1475 -> 1515. Before that: documentation refresh across `docs/` (focus.md, resume.md, playbook.md, progress.json, INVENTORY.md, training/Training_System.md), which had all still described Stage 1 as the current stage five weeks after Stage 2A started on 2026-07-20, and still gave the gate as `python tests.py` → 141/141. Four Known Open Issues rows below were also false and are corrected: the Stage 2 plan is built, the `Training plan/` duplicate is gone, the biomechanical review was done 2026-07-19, and Strength BioAge is no longer dormant. Before that: adding NAP support — `biometrics.split_sleep_periods`/`dedupe_sleep_periods`/`NAP_MIN_SECONDS`, the Day-total panel on the Home Sleep drill-down, and the duration-counts-naps / architecture-does-not split that `services/sleep_score.py` depends on. Before that: body-temperature deviation as a fourth `engine.traffic_light` metric (absolute °C cut points, not a rolling baseline), the `engine.baseline_drift` guard, full Stage 1 coverage in `training_constants.EXERCISE_MOVEMENT_WEIGHT` (Strain/ACWR were counting rehab drills as loaded lifting), and the `sessions.movement_category` mislabelling fix. Before that: Oura+Garmin MOVEMENT fusion (`services/sleep_movement.py`, the movement tick strip on the Home Sleep drill-down, `sleep_fusion.RULES_VERSION` 2's movement-aware staging rules, and the Oura `movement_30_sec`/HR/HRV and Garmin `sleepMovement`/HR/stress columns). Before that: Oura+Garmin sleep-stage fusion (`services/sleep_fusion.py`, the Garmin Sleep Stages and Sleep Fusion tabs, Garmin 429 backoff + circuit breaker). Previously: heart-rate-derived strain (`services/hr_load.py` — Edwards' TRIMP — and `services/hr_matching.py`), true per-set training capture, readiness-based auto-shift session scheduling (`services/scheduling.py`), double-progression weight/rep tracking, weekly tonnage (`services/volume.py`), Sleep Debt scoring, and the per-night wake-time adjustment.*
...
Expected: **1584/1584 passed** (or higher — this count grows as tests are added; treat it as a floor, not an exact match)
...
1. `python -m pytest tests/` → 1584/1584 (or higher if new tests were added)
...
services/ — framework-agnostic backend + business logic. ZERO Streamlit
  ...
  Pure logic:      engine.py · readiness.py · stats.py · rules.py · ai.py ·
                    ...
                    flexibility.py (the Flexibility screen's backend. Scores
                    SKILLS, never regions, and the score of a skill is
                    `min(rungs)` with the limiting rung NAMED beside it —
                    averaging is what v1 did and it is exactly how a broken
                    capacity hides behind healthy ones. Each rung is measured
                    PASSIVE / ISOMETRIC / ACTIVE in one position; the rung
                    scores off ACTIVE where it exists, and `gap =
                    passive − active` decides whether it needs RANGE or
                    STRENGTH, which is the question v1 could not answer at
                    all. Read the module docstring before adding anything: it
                    names five deleted symbols that a test brings the gate
                    down over if they reappear, and refuses a flexibility age,
                    any averaging of rungs, and filling an unmeasured rung
                    from a neighbour) ·
...
Reference data:
  ...
  flexibility_baselines.py — the four scored SKILLS, their ladders, and the 14
                           rung tests: each rung's full protocol (setup, LOCK,
                           measurement), its 0-anchor and 100-anchor, and
                           whether it `replaces` a standard test that is
                           contraindicated for this athlete. `ASSESSMENTS` is
                           empty — no rung has been measured yet, and the 22
                           legacy pose ratings under LEGACY_* are provenance
                           that nothing computes from. Editing an anchor
                           re-scores every stored reading, so treat the
                           anchors the way strength_baselines.py's peaks are
                           treated.
...
tests/       — pytest suite (1584 tests), the sole deterministic gate
...
| Issue | Status |
|-------|--------|
| **⏳ Flexibility has ZERO measured rungs — `ASSESSMENTS` is empty** | Open by design, and it is the only thing standing between this sector and being useful. The screen renders its empty state and the 14-step capture flow is built, tested and resumable; nothing has been run against the athlete's body yet. **The 22 legacy pose ratings do not fill it** — v2 asks passive/isometric/active in a locked position and a 1-100 self-rating of a yoga pose answers none of the three, so `LEGACY_POSE_DEPTH_RATINGS_2026_08_05` is provenance, not data. Until the first assessment exists, `WIDE_GAP_POINTS` (25.0) is set from the general hypermobility literature rather than from this athlete, and every skill reads unmeasured. **Measure COLD** — no warm-up, ever, or the reading tracks the morning's stretching rather than durable change. |
| **Three frozen constants are not captured, and three tests need them** | Open. Acromion-to-styloid length, shin length, and a traced foot outline are per-athlete constants that several rung tests normalise against — measure once, store, reuse forever. They are frozen precisely so a later re-measure cannot silently re-scale history. Capture them at the same session as the first assessment; the three tests that need them cannot be run first. |
| **The app offers yoga on rest days, the worst window for adaptation** | **Decided 2026-08-06, deliberately NOT built: option 2, an `intent` field on `YogaSession` (`restorative` \| `training`).** Rest days get restorative flows only; adaptation-seeking flexibility work is offered on good windows. Nothing in the code distinguishes the two today, so the adaptation window cannot be shown without either being wrong or contradicting the button beside it. Deferred by athlete decision until the **training-schedule overhaul** — full scheduler integration is the real fix, and other features come first. Do not wire flexibility into `services/scheduling.py` before that. |
| **The `lats` anchor (160°) is provisional, and says so** | Open. The published supine lat-length norm is defined against a **flat** lumbar spine; this test uses **full posterior tilt** (hips/knees 90°, feet on a wall) to maximise lat tension and remove the hands from the measurement. Those are not the same position, so the norm does not transfer cleanly and the 100-anchor is an informed estimate. Re-derive it once paired readings exist, or ask the physio to set it on 2026-08-16. The rung is still worth having unanchored: it is the ONLY thing that separates a lat restriction from a pec or capsular one, and post-Latarjet that distinction changes the prescription from "stretch harder" to "do not". |
