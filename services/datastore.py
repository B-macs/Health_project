...
def _insert_rows(conn: sqlite3.Connection, table: str, rows: list[dict], replace: bool = False) -> int:
    ...
    Notion-backed table (readiness_checkins) where a pre-existing,
    not-yet-merged duplicate Notion page for the same date is a known
    possible data-quality issue (see scripts/merge_duplicate_checkins.py)
    rather than a bug in this module -- last-one-wins there rather than raising and rolling back the
